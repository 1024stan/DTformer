...
def main(args):
    ...
    for epoch in range(args.mask_epochs):
        for i, mask_data in enumerate(mask_database):
            # mask_list = range(-6:-1)
            # 替换
            for location in range(36, max_len, 1):
    ...
    train_masked_database_txt_name = args.fine_tune_data_save_path + '/train.txt'
    test_masked_database_txt_name = args.fine_tune_data_save_path + '/test.txt'
    all_masked_database_txt_name = args.fine_tune_data_save_path +  '/all.txt'
...
if __name__ == '__main__':
    ...
    # parser.add_argument('--mask_rate', type=str, default=0.2, help='添加掩码的比例')
    parser.add_argument('--mask_epochs', type=str, default=1, help='添加掩码的epoch')
    ...
    parser.add_argument('--fine_tune_data_save_path', type=str, default='./data/fine_tune_data', help='存放掩码后数据的路径')
